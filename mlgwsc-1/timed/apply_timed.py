...
class TimedSequential(torch.nn.Module):
    def __init__(self, modules, *args, **kwargs):
        torch.nn.Module.__init__(self, *args, **kwargs)
        self.modules = list(modules)
        self.times = []
        for i, mod in enumerate(self.modules):
            self.add_module('%03i'%i, mod)
            self.times.append([])
        self.times.append([])
    def forward(self, x):
        for subtimes, mod in zip(self.times[:-1], self.modules):
            time0 = time.perf_counter_ns()
            x = mod(x)
            time1 = time.perf_counter_ns()
            subtimes.append(time1-time0)
        self.times[-1].append(x.shape[0])
        return x
    def save_times(self, times_file):
        np.savetxt(times_file, np.array(self.times).T)
...
def get_base_network(path=None, device=default_device, detectors=1):
    ...
    i = 0
    while not 'Flatten' in network[i].__str__():
        i += 1
    modules = [network[:i], network[i], network[i+1:]]
    return TimedSequential(modules)
...
def get_triggers(weights_path, coincident, usr, inputfile, step_size=0.1,
    ...
    times_file : path where the network's convolutional part's
        and classifier's processing times will be stored.
    ...
    # test_input = torch.randn(32, 2, 2048).to(device=device)
    ...
    Network.base_network.save_times(times_file)
...
def main():
    ...
    parser.add_argument('--times-output', type=str, default=None, help="Path to a file where computation times of the network's convolutional part and classifier will be stored. Optional.")
